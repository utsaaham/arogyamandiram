...
export default function HealthPlanLogView({
  log,
  openSections,
  onToggleSection,
  allExpanded,
}: {
  log: HealthPlanDebugLog;
  ...
  const inputJson = JSON.stringify(log.userRequest ?? {}, null, 2);
  const parsedJson = JSON.stringify(log.parsedResult ?? {}, null, 2);
  const outputJson = JSON.stringify(
    {
      clampedTargets: log.clampedTargets ?? null,
      explanations: log.explanations ?? null,
    },
    null,
    2
  );
    ...
    <div className="flex flex-col items-center">
      <div className="w-full">
        <PipelineZone label="Input" open={isOpen('input')} onToggle={() => handleToggle('input')}>
          <Step2ResponseBlock raw={inputJson} isJson={true} />
        </PipelineZone>
      </div>
      ...
      <div className="w-full">
        <PipelineZone label="Step 2 · Response" open={isOpen('step2')} onToggle={() => handleToggle('step2')}>
          ...
          <StepDivider />
          <div>
            <p className="mb-1.5 text-[10px] font-medium uppercase tracking-widest text-zinc-500">Parsed Output</p>
            <div className="rounded border border-[#1e1e1e] bg-black/20 p-3">
          ...
          <StepDivider />
          <div>
            <p className="mb-1.5 text-[10px] font-medium uppercase tracking-widest text-zinc-500">Clamped Targets + Explanations</p>
            <div className="rounded border border-emerald-500/30 bg-emerald-950/20 p-3">
              <Step2ResponseBlock raw={outputJson} isJson={true} />
            </div>
          </div>
        </PipelineZone>
      </div>
